...
theorem foldl_H_zeros
    (n : Nat)
    : (List.replicate n '0').foldl (fun a c => a * 10 + (c.toNat - 48)) 0 = 0
    := by
  induction n with
  | zero => simp
...
theorem decodeNumberBytes?_congr
    ...
    : decodeNumberBytes? arr1 q1 q1' = decodeNumberBytes? arr2 q2 q2'
    := by
...
theorem decode_at
    ...
    : decodeNumberBytes? arr q (q + s.toUTF8.size) = some v
    := by
