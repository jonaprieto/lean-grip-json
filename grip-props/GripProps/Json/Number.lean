...
theorem numByte_digit
    ...
    : numByte st (UInt8.ofNat c.toNat) =
      (if st.phase = 1 then
        ...
        else if st.phase = 0 then
          { st with mant := st.mant * 10 + (c.toNat - 48) }
        else
          { st with expVal := st.expVal * 10 + (c.toNat - 48) }) := by
...
theorem foldl_numByte_frac
    ...
    : ds.foldl (fun s c => numByte s (UInt8.ofNat c.toNat)) st =
      { st with
        mant := ds.foldl (fun a c => a * 10 + (c.toNat - 48)) st.mant,
...
theorem foldl_numByte_frac_ascii
    ...
    : (cs.flatMap String.utf8EncodeChar).foldl numByte st =
      { st with
        mant := cs.foldl (fun a c => a * 10 + (c.toNat - 48)) st.mant,
...
theorem numByte_dot
    (st : NState)
    : numByte st (UInt8.ofNat ('.').toNat) = { st with phase := 1 } :=
  rfl

/-- The Horner fold over leading zeros starting from `0` stays `0`. -/
theorem foldl_H_zeros
    (n : Nat)
...
theorem decodeNumberBytes?_congr
    {arr1 arr2 : ByteArray}
    {q1 q1' q2 q2' : Nat}
...
theorem decode_renderNumScientific_at
    (arr : ByteArray)
    (q : Nat)
    (m : Int)
    (e : Nat)
    ...
    (hm :
      ∀ j, j < (renderNumScientific m e).toUTF8.size →
